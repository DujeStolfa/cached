...
class WalletTransactionGraph extends StatefulWidget {
  final Wallet selectedWallet;

  const WalletTransactionGraph({Key key, this.selectedWallet})
      : super(key: key);

  @override
  _WalletTransactionGraphState createState() => _WalletTransactionGraphState();
}

class _WalletTransactionGraphState extends State<WalletTransactionGraph> {
  ...
  LineChartData mainData(PredictionApiResponse prediction) {
    ...
    predicted = prediction.prediction[0] -
        _transactionData[0][_transactionData[0].length - 1];

    for (var i = 0; i < prediction.prediction.length; i++) {
      predicted = prediction.prediction[i] -
